...
def to_vmec(self, filename, r=0.1, params=dict(), ntheta=20, ntorMax=14):
    ...
    Args:
        ...
        r (float):  near-axis radius r of the desired boundary surface
    ...
    # Set current profile:
    I2 = self.I2.detach().numpy().item()
    ncurr = 1
    pcurr_type = 'power_series'
    ac = [1]
    curtor = 2 * np.pi / mu0 * I2 * r * r
    ...
    file_object.write('! Near-axis parameters:  radius r = '+str(r)+', etabar = '+str(self.etabar.detach().numpy())+'\n')
    file_object.write('! nphi = '+str(self.nphi)+', order = '+self.order+', sigma0 = '+str(self.sigma0)+', I2 = '+str(I2)+', B0 = '+str(self.B0)+'\n')
    ...
    file_object.write('  LASYM = '+str(self.lasym.detach().numpy())+'\n')
    ...
    file_object.write('  RAXIS_CC = '+str(self.rc.detach().numpy())[1:-1]+'\n')
    if self.lasym:
        file_object.write('  RAXIS_CS = '+str(-self.rs.detach().numpy())[1:-1]+'\n')
        file_object.write('  ZAXIS_CC = '+str(self.zc.detach().numpy())[1:-1]+'\n')
    file_object.write('  ZAXIS_CS = '+str(-self.zs.detach().numpy())[1:-1]+'\n')
