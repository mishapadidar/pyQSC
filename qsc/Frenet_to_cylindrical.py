...
def Frenet_to_cylindrical(self, r, ntheta=20):
    ...
    for j_theta in range(ntheta):
        ...
        for j_phi in range(nphi_conversion):
            ...
            # bound choice is heuristic; i think we can subtract up to np.pi / self.nfp
            phi0_rootSolve_min = phi_target - np.pi / (2 * self.nfp)
            phi0_rootSolve_max = phi_target + np.pi / (2 * self.nfp)
